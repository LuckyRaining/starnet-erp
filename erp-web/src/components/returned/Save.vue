...
export default {
  ...
  watch: {
    ...
    // 结算信息联动：优惠三字段互算 + 退款/欠款互算 + 客户承担费用=本次退款
  ...
  data() {
    return {
      ...
      // current: 以本次退款为主输入；customerFee: 以客户承担费用为主输入；debt: 以本次欠款为主输入（二者始终相等）
  ...
  methods: {
    ...
    // 公式：本次欠款 = 优惠后金额 - 本次退款（客户承担费用与本次退款相等）
    ...
    // 公式：本次欠款 = 优惠后金额 - 客户承担费用（客户承担费用与本次退款相等）
    ...
    // 公式：本次退款 = 优惠后金额 - 本次欠款（客户承担费用与本次退款相等）
